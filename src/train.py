...
# Add VALLA directory to sys.path
# Adapted from Taras Alenin's answer on StackOverflow at:
# https://stackoverflow.com/a/55623567
valla_path = os.path.join('..', 'VALLA')
...
def train(view_path,
    ...
    for fold_idx in range(num_folds):
        # Clear GPU cache at the end of each fold
        if device == "cuda":
            torch.cuda.empty_cache()
        ...
        train_dataset = VALLAAVTrainDataset(view_path)
        val_dataset = VALLAAVValDataset(view_path.replace('pan20_train', 'pan20_val'))
