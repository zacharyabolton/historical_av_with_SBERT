...
def train(batch_size,
    ...
    # Instantiate the LILA dataset and data loader
    train_ds = LILADataset('../data/normalized/undistorted',
    ...
    We are pre-tokenizing and then reducing to appropriate lengths, rather
    than tokenizing a single payload as this warning is assuming.
    """)
    ...
    # Create list to store batch processing durations for computational
    ...
    try:
        # Iterate over the dataloader one batch at a time
        for batch_idx, (batch_a, batch_b, labels) in enumerate(
              dataloader):
            # batch_content is a tuple containing three elements coming
            # from the PyTorch `DataLoader` object:
            # - batch_a at index 0 - the batch tensor of chunks to be fed
            #   through the 'left' side of the Siamese network.
            # - batch_b at index 1 - the batch tensor of chunks to be fed
            #   through the 'right' side of the Siamese network.
            ...
            if batch_idx % 10 == 0:
                ...
                print(f'Batch {batch_idx}, '
                      ...
                      f'Avg Time/Batch: {avg_duration:.2f}s')
        ...
        if durations:
            ...
            # average batch processing calculation innacurately. This also
            # has a benifit of letten us calculate the average with index
            # without creation a division by zero error
...
def run_training_loop(args):
    ...
    filename = f"{'_'.join(current_time.split())}_losses.csv"
    ...
    # Save run metadata
    filename = f"{'_'.join(current_time.split())}_hyperparameters.txt"
    filepath = os.path.join(output_dir, filename)
    with open(filepath, 'w') as f:
